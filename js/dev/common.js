...
(() => {
  ...
  const closeAll = () => {
    hotspots.forEach((h) => h.removeAttribute("data-open"));
  };
  const onHotspotClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    const hotspot = e.currentTarget;
    const isOpen = hotspot.hasAttribute("data-open");
    closeAll();
    if (!isOpen) {
      hotspot.setAttribute("data-open", "");
    }
  };
  hotspots.forEach((h) => {
    h.addEventListener("click", onHotspotClick);
    h.addEventListener("touchstart", onHotspotClick);
  });
...
if (document.querySelector(".filter-catalog__title")) {
  document.querySelector(".filter-catalog__title").addEventListener("click", function(e) {
    if (window.innerWidth < 900) {
      document.querySelector(".filter-catalog__spollers").classList.toggle("--active");
    }
  });
}
export {
